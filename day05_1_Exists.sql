/*=========================== EXISTS, NOT EXIST ================================
	...
	WHERE [NOT] EXISTS (SELECT kolon_ismi FROM tablo_ismi WHERE koşul); 
==============================================================================*/
...
select * from nisan;
 ...
 /* -----------------------------------------------------------------------------
  SORU1: MART VE NİSAN aylarında aynı URUN_ID ile satılan ürünlerin URUN_ID’lerini ve aynı zamanda
  bu ürünleri MART ayında alan MUSTERI_ISIM' lerini listeleyen bir sorgu yazınız. 
 -----------------------------------------------------------------------------*/  
-- select urun_id, musteri_isim from mart where exist(ikinci sorgu);
-- select urun_id from nisan where mart.urun_id = nisan.urun_id;   -- ikinci sorgu
...
-----------------------------------------------------------------------------*/ 
-- select urun_isim, musteri_isim from nisan where exists(ikinci sorgu);
-- select urun_id from mart where nisan.urun_id=  mart.urun_id;   -- ikinci sorgu
...
/* SORU3: Her iki ayda ortak satilmayan ürünlerin URUN_ISIM'lerini ve bu ürünleri
  NİSAN ayında satın alan MUSTERI_ISIM'lerini listeleyen bir sorgu yazınız. 
 -----------------------------------------------------------------------------*/
-- select urun_isim, musteri_isim from nisan where not exists(ikinci sorgu);
-- select urun_id from mart where nisan.urun_id=  mart.urun_id;   -- ikinci sorgu

select urun_isim, musteri_isim from nisan
where not exists(select urun_id from mart where nisan.urun_id=  mart.urun_id);
...
-- SORU7: tabloyu eski haline döndürün (ismi 'NO NAME' olanlari null yap)
...
SORU8:   isim 'i NULL olanlara 'Henuz isim girilmedi'
         ...
         gibi ifade yazmamak için;
         coalesce = birleşmek
*/
update insanlar set isim = coalesce('Henuz isim girilmedi'),
					adres = coalesce('Henuz adres girilmedi'),
                    ssn = coalesce('no ssn')
