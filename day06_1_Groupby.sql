...
select urun_adi, count(isim) as toplam_isim from manav where urun_adi is not null group by urun_adi;
...
where urun_adi is not null group by urun_adi order by max desc;
